...
class FactorialRecurrenceExample(Scene):
    def construct(self):
        # Title
        title = Text("Visualization of the Factorial Recurrence Relation\n\n", font_size=30)
        title.to_edge(UP)
        ...
        for i in range(1, 5):
            ...
            rest_of_line = Text(f"{lines[i][7:]}", font_size=24).next_to(part_to_move, RIGHT)
...
#    pip install manim
# 2. Save the code in a Python file, for example, factorial_recurrence_example.py.
# 3. Run the Manim command to render the animation:
#    manim -pql factorial_recurrence_example.py FactorialRecurrenceExample
