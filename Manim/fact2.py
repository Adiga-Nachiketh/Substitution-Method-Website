...
class RecurrenceRelation(Scene):
    def construct(self):
        # Title
        title = Text("Visualization of Factorial Recurrence Relation\n\n", font_size=30)
        title.to_edge(UP)
        ...
        lines = [
            ("T(n-3) = (n-3) * T(n-4)", "T(n-3) = (n-3) * (n-4) * T(n-5)"),
            ("T(n-2) = (n-2) * T(n-3)", "T(n-2) = (n-2) * (n-3) * (n-4) * T(n-5)"),
            ("T(n-1) = (n-1) * T(n-2)", "T(n-1) = (n-1) * (n-2) * (n-3) * (n-4) * T(n-5)"),
            ("T(n) = n * T(n-1)", "T(n) = n * (n-1) * (n-2) * (n-3) * (n-4) * T(n-5)")
        ]
